...
from tests.pytest_plugins.capsql.expression import SQLExpression


class SQLAlchemyCapturer:
    """
    The main fixture class for the capsql plugin. Used to perform asserts
    ...
    Use with the capsql fixture. See the tests for the capsql plugin for example usage
    """
    ...
    def __exit__(
        ...
        if self.partial_context is None:
            raise RuntimeError(
                f"{self.__class__.__name__}: attempting to call __exit__ before __enter__"
            )
    ...
    def assert_query_count(
        ...
        *,
        ...
        assert (
            expected_query_count == actual_query_count
